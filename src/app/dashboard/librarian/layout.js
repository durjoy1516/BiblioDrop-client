...
import { LayoutDashboard, PlusCircle, BookOpen, Truck } from "lucide-react";
...
export default function LibrarianLayout({ children }) {
  ...
  const navItems = [
    ...
    { name: "Manage Inventory", href: "/dashboard/librarian/inventory", icon: BookOpen },
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-950 text-slate-100">
      ...
      <aside className="w-full md:w-64 bg-slate-900 border-r border-slate-800 p-4 space-y-6">
        <div className="text-xl font-bold text-amber-500 px-2 flex items-center gap-2">
          <BookOpen className="w-6 h-6" /> Librarian Portal
        </div>
